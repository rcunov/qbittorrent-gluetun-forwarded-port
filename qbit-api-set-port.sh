...
if [ "${logLevel}" != "DEBUG" ] && [ "${logLevel}" != "INFO" ] && [ "${logLevel}" != "ERROR" ]; then
  ...
  timestamp=$(date +%Y-%m-%dT%H:%M:%S%z)
  echo "{\"timestamp\": \"$timestamp\", \"log_level\": \"ERROR\", \"message\": \"$message\"}" # Manually create the log output since an invalid log level breaks the logging function
  exit 1
fi
...
#  log "gtResponse: ${gtResponse}" DEBUG
  ...
  if [[ "${curlOutput}" == *2?? ]]; then
    ...
    log "$(cat qbitAuthResp.txt)" ERROR
    exit 1
  fi
  ...
  if [ ${gtForwardedPort} -ne ${qbListenPort} ]; then
    ...
    log "qBit listening port is already configured properly. Currently listening on ${gtForwardedPort}" DEBUG
  fi
